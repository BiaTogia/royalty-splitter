...
artist1, created1 = UserAccount.objects.get_or_create(
...
artist2, created2 = UserAccount.objects.get_or_create(
...
artist3, created3 = UserAccount.objects.get_or_create(
...
# Delete any previous test track
Track.objects.filter(title="Collaborative Masterpiece").delete()
...
Split.objects.filter(track=track).delete()
...
payouts = Payout.objects.filter(wallet__in=[wallet1, wallet2, wallet3]).order_by('-txn_date')[:3]

print(f"  Recent Payouts: {payouts.count()}")
...
final_payouts = Payout.objects.filter(wallet__in=[wallet1, wallet2, wallet3]).order_by('-txn_date')[:3]
